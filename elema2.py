...
keywordExcel = "D:\worksapce\python\爬虫饿了么\\keyword.xlsx"  # 关键字检索外卖地点保存路径
...
def reqsetting():  # 首先构造请求头headers，url目前暂时保存根路径
    ...
    req = urllib2.Request(url=weburl, headers=webheaders)
...
if __name__ == '__main__':  # 程序运行入口
    ...
    for keyword in keywords:  # 遍历关键字集合，构造不同的请求参数，附加到URL 请求上
        ...
        params = urllib.urlencode(params)  # 将请求参数进行编码
        req.full_url = newUrl + params  # 重新构造请求参数

        webpage = urllib.urlopen(req)  # 获取数据
